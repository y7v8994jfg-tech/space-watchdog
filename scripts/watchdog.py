...
Status check: no token needed (Space is public)
Restart: requires HF_TOKEN
"""
import os
import sys
import json
...
def check_runtime():
    """Check Space runtime without token (Space is public)"""
    try:
        r = requests.get(f"{HF_API}/{REPO}/runtime", timeout=15)
...
def main():
    ...
    stage, err, hw_current = check_runtime()
    ...
    if needs_restart:
        ...
        if not HF_TOKEN:
            print(f"  [SKIP] No HF_TOKEN - cannot restart. Set GitHub Secret HF_TOKEN.")
            sys.exit(1)
        ...
        if status == 200:
            ...
            print(f"  Trying factory reboot...")
            fstatus, fbody = factory_reboot(HF_TOKEN)
            print(f"  Factory reboot: HTTP {fstatus}")
    else:
